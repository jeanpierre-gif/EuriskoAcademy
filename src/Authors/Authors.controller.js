...
createAuthor = async (req, res) => {
  try {
    ...
    console.error(err);
    return res.status(500).json({ success: false, message: err.message,details: err.details });
  }
};
...
updateAuthorById = async (req, res) => {
  try {
    ...
    console.log(err);
    res.status(500).json({ success: false, message: err.message,details: err.details });
  }
};
